...
<!--
    ============================================================
    AI ASSISTANT: STOP AND READ BEFORE EDITING THIS FILE
    ============================================================
    Check your MEMORY RULES before making changes. If you cannot
    see them, ask the user: "Can you confirm my memory rules for
    editing shared files?"
    
    Always use user-uploaded files as working base, never 
    /mnt/project/ files which may be outdated.
    ============================================================
-->
<html>
<head>
    ...
    <style>
        :root {
            /* All px values derived from mm constants / SCALE_FACTOR (2.5)
               FRAME_THICKNESS_MM = 45  -> 18px
               SASH_BORDER_MM     = 16  -> 6.4px
               SASH_FACE_MM       = 110 -> 44px
               GLASS_BORDER_MM    = 16  -> 6.4px
               MULLION_WIDTH_MM   = 90  -> 22.5px
               midrail halves are set dynamically by pvcResiDoor_applyMidrailHeights()
            */
            --panel-width: 360px;        /* 900mm / 2.5 */
            --panel-height: 280px;       /* 700mm / 2.5 */
            --measure-width: 100px;      /* UI chrome - not a product dimension */
            --frame-thickness: 18px;     /* FRAME_THICKNESS_MM 45 / 2.5 */
            --sash-border: 6.4px;        /* SASH_BORDER_MM 16 / 2.5 */
            --sash-inset: 37.6px;        /* (SASH_FACE_MM 110 - SASH_BORDER_MM 16) / 2.5 = 94/2.5 */
            --glass-border: 6.4px;       /* GLASS_BORDER_MM 16 / 2.5 */
            --midrail-1-half: 23.2px;    /* default 116mm/2.5/2 - overridden by JS */
            --midrail-2-half: 0px;       /* midrail 2 off by default */
            --mullion-width: 36px;       /* MULLION_WIDTH_MM 90 / 2.5 */
        }
        
        body { margin: 0; padding: 0; overflow: hidden; height: 100vh; }

        /* Auto-scale wrap — fills full viewport, centres the product */
        #product-scale-wrap {
            width: 100%;
            height: 100vh;
            overflow: hidden;
        ...
        .wrapper {
            ...
            /* transform: scale() set dynamically by pvcResiDoor_autoScale() */
        }
        
        /* Top width measurement */
        .top-measure {
            margin-left: calc(var(--measure-width) + 10px);  /* Align with panel (measure + panel margin) */
        ...
        /* Wrapper for shadow effect - positioned over panels only */
        .pvc-resi-door-frame-border-shadow {
            position: absolute;
            left: calc(var(--measure-width) + 8px);
            top: 0;
            bottom: 0;
            width: var(--panel-width);
        ...
        /* Single sash overlay spanning all rows — for open-out shadow */
        .pvc-resi-door-sash-overlay {
            position: absolute;
            pointer-events: none;
            z-index: 200;
            display: none;
        ...
        .row-measure.first::before {
            display: block;
        }
        
        /* Bottom cap - all rows */
        .row-measure .cap-bottom {
            position: absolute;
            left: calc(50% - 5px);
            bottom: 0;
            width: 10px;
        ...
        /* Frame edges - outer frame (first-group equivalent) */
        ...
        .pvc-resi-door-frame-leftEdge { }
        .pvc-resi-door-frame-rightEdge { }
        ...
        .pvc-resi-door-frame-topEdge { top: 0; }
        .pvc-resi-door-frame-bottomEdge { bottom: 0; }
        
        /* Elements that only show when row is the bottom row */
        .bottom-row-only {
            display: none;
        }
        
        /* Sash area (second-group equivalent) */
        .sash-area {
            flex: 1;
            background: #efeeee;
            position: relative;
            box-sizing: border-box;
            /* Left/right borders on all rows */
            border-left: var(--sash-border) solid #ffffff;
            border-right: var(--sash-border) solid #a0a0a0;
        }
        
        .pvc-resi-door-sash-panel1 { }
        .pvc-resi-door-sash-panel2 { }
        .pvc-resi-door-sash-panel3 { }
        
        /* Row 1: has top sash border */
        .pvc-resi-door-sash-panel1 {
            border-top: var(--sash-border) solid #ffffff;
            margin-top: var(--frame-thickness);
        }
        /* Row 3: has bottom sash border */
        .pvc-resi-door-sash-panel3 {
            border-bottom: var(--sash-border) solid #a0a0a0;
            margin-bottom: var(--frame-thickness);
        }
        /* Row 2 and Row 3: no top sash border (handled by midrail or dynamically) */
        /* Bottom sash border and margin are applied dynamically via JS */
        
        /* Glass content area inside sash - flex container for panes */
        .glass-area {
            position: absolute;
            top: calc(var(--sash-inset) + var(--sash-border));
            left: calc(var(--sash-inset) + var(--sash-border));
            right: calc(var(--sash-inset) + var(--sash-border));
            bottom: calc(var(--sash-inset) + var(--sash-border));
            display: flex;
            flex-direction: row;
            overflow: visible;
            cursor: pointer;
        ...
        .glass-pane {
            flex: 1;
            background: black;
            border-top: var(--glass-border) solid #a0a0a0;
            border-left: var(--glass-border) solid #a0a0a0;
            border-right: var(--glass-border) solid #ffffff;
            border-bottom: var(--glass-border) solid #ffffff;
            position: relative;
        }
        
        /* Mullion */
        .mullion {
            width: var(--mullion-width);
            background: #efeeee;
            flex-shrink: 0;
            position: relative;
            cursor: ew-resize;
        ...
        /* Mullion extensions (weld lines) */
        .mullion-ext {
            position: absolute;
            left: 0;
            width: 100%;
            height: var(--mullion-ext, 7px);
            background: inherit;
            z-index: 10;
        }
        
        .mullion-ext.top { top: calc(-1 * var(--mullion-ext, 7px)); }
        .mullion-ext.bottom { bottom: calc(-1 * var(--mullion-ext, 7px)); }
        
        /* Midrail end weld lines */
        .midrail-end {
            position: absolute;
            width: var(--sash-inset);
            z-index: 5;
            background: #efeeee;
        }
        
        /* Midrail 1 (between row 1 and row 2) */
        #row1 .midrail-end.bottom,
        #row2 .midrail-end.top {
            height: var(--midrail-1-half);
        }
        
        /* Midrail 2 (between row 2 and row 3) */
        #row2 .midrail-end.bottom,
        #row3 .midrail-end.top {
            height: var(--midrail-2-half);
        }
        
        .midrail-end.left { left: var(--sash-border); }
        .midrail-end.right { right: var(--sash-border); }
        
        .midrail-end.bottom { bottom: 0; }
        .midrail-end.top { top: 0; }
        ...
        /* Door sash corner weld lines */
        .pvc-resi-door-sash-corner-line {
            position: absolute;
            width: calc(var(--sash-inset) + var(--sash-border));
            height: calc(var(--sash-inset) + var(--sash-border));
            pointer-events: none;
            background: transparent;
            z-index: 10;
        }
        
        .pvc-resi-door-sash-corner-line.top-left { top: 0; left: 0; }
        .pvc-resi-door-sash-corner-line.top-right { top: 0; right: 0; }
        .pvc-resi-door-sash-corner-line.bottom-left { bottom: 0; left: 0; }
        .pvc-resi-door-sash-corner-line.bottom-right { bottom: 0; right: 0; }
        
        /* Outer frame corner weld lines */
        ...
        /* Clickable overlay for frame corner toggle - larger hit area */
        .frame-corner-toggle {
            position: absolute;
            top: 0;
            left: 0;
        ...
        .frame-corner-toggle:hover {
            background: rgba(2, 153, 138, 0.2);
        }
        
        /* Clickable overlay for sash corner toggle - larger hit area */
        .sash-corner-toggle {
            position: absolute;
            top: 0;
            left: 0;
        ...
        .sash-corner-toggle:hover {
            background: rgba(2, 153, 138, 0.2);
        }
        
        /* Clickable overlay for rail joint toggle - larger hit area */
        .rail-joint-toggle {
            position: absolute;
            cursor: pointer;
            z-index: 250;
        ...
        .rail-joint-toggle:hover {
            background: rgba(2, 153, 138, 0.2);
        }
        
        /* Mullion toggle - positioned at top of mullion */
        .rail-joint-toggle.mullion-toggle {
            width: 22px;
            height: 12px;
            top: -10px;
            left: 50%;
        ...
        /* Midrail toggle - positioned to cover both halves of midrail */
        .rail-joint-toggle.midrail-toggle {
            width: 18px;
            height: 18px;
            left: 0;
            bottom: -9px;
        ...
        .pvc-resi-door-frame-corner-line.top-left { top: 0; left: 0; }
        .pvc-resi-door-frame-corner-line.top-right { top: 0; right: 0; }
        .pvc-resi-door-frame-corner-line.bottom-left { bottom: 0; left: 0; }
        .pvc-resi-door-frame-corner-line.bottom-right { bottom: 0; right: 0; }
        ...
        .rows-container.cill-visible .cill-row { display: flex; }
        
        .cill-measure {
            width: var(--measure-width);  /* Same as row-measure, cill will overhang via its own width */
            margin: 0;
            padding: 0;
        }
        
        /* Cill — width set dynamically by JS as widthPx + (FRAME_THICKNESS_MM/SCALE_FACTOR)*2 */
        .cill {
            height: 0;
            background: #efeeee;
            ...
            box-sizing: border-box;
            position: relative;
            width: calc(var(--panel-width) + 32px);  /* approximate seed — overridden by JS */
        }
        
        /* Trickle Vent - Frame Head */
        .pvc-resi-door-trickle-vent {
            position: absolute;
            height: 6px;
            top: 2px;
            left: 50%;
            ...
            box-shadow: 0 2px 4px rgba(172, 172, 173, 0.3);
            display: none;
        }
        
        /* Trickle Vent - Door Head (inside sash, above glass) */
        .pvc-resi-door-sash-vent {
            position: absolute;
            height: 6px;
            top: 4px;
            left: 50%;
        ...
        /* Remove frame bottom border when cill is visible (JS handles this dynamically) */
        /* The actual border removal is handled by pvcResiDoor_updateBottomRowBorder() */
        
        /* Adjust glass-area for midrails.
           Sash edges: calc(--sash-inset + --sash-border) — already set in base rule.
           Midrail edges: --midrail-half only — no sash-border, midrail is not a sash edge. */
        #row1 .glass-area {
            bottom: var(--midrail-1-half);
        }
        #row2 .glass-area {
            top: var(--midrail-1-half);
            bottom: var(--midrail-2-half);
        }
        #row3 .glass-area {
            top: var(--midrail-2-half);
        }
        
        /* Right total height measurement */
        .right-measure {
            width: var(--measure-width);
            position: relative;
            display: flex;
        ...
        /* Draw-to-split cursor on glass areas */
        .glass-area {
        ...
        /* Midrail drag handle — overlay over the midrail gap for dblclick thickness popup */
        .midrail-drag-handle {
            position: absolute;
            left: 0;
            right: 0;
        ...
        /* Draw-to-split popup (same pattern as casement cf-sash-popup) */
        .rd-popup-overlay {
            position: fixed; top: 0; left: 0; right: 0; bottom: 0;
            background: rgba(0,0,0,0.4); z-index: 500;
        }
        .rd-popup {
            position: fixed; background: #1a1a2e; border: 1px solid #16213e;
            border-radius: 6px; padding: 20px; box-shadow: 0 6px 20px rgba(0,0,0,0.3);
            z-index: 1000; min-width: 240px; left: 50%; top: 50%;
            transform: translate(-50%,-50%); font-family: Arial, sans-serif; color: #e0e0e0;
        }
        .rd-popup h3 {
            margin: 0 0 15px 0; color: #02998a; font-size: 14px;
            font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px;
        }
        .rd-popup-close {
            position: absolute; top: 10px; right: 10px; width: 25px; height: 25px;
            border: none; background: transparent; cursor: pointer; font-size: 25px;
            color: #888; padding: 0; display: flex; align-items: center; justify-content: center;
        }
        .rd-popup-close:hover { color: #fff; background: #2a2a4a; border-radius: 4px; }
        .rd-thickness-options { display: flex; gap: 8px; flex-wrap: wrap; }
        .rd-thickness-btn {
            min-width: 52px; height: 36px; border: 1px solid #2a2a4a; background: #2a2a4a;
            cursor: pointer; display: flex; align-items: center; justify-content: center;
            border-radius: 4px; color: #ccc; font-size: 12px; font-weight: 600; padding: 0 8px;
        }
        .rd-thickness-btn:hover { background: #3a3a5a; border-color: #4a4a6a; }
        .rd-thickness-btn.active { background: #02998a; border-color: #02998a; color: white; }
        .rd-popup-footer {
...
<div class="wrapper">
        <div class="top-measure">
            ...
            <input type="text" value="900" onchange="pvcResiDoor_updateWidth(this.value)">
        </div>
        
        <div class="main-area">
            <div class="rows-container">
                <!-- Shadow overlay for unified door shadow -->
                <div class="pvc-resi-door-frame-border-shadow" id="pvcResiDoorFrameBorderShadow"></div>
                <!-- Sash open-out overlay — single div spanning all rows -->
                <div class="pvc-resi-door-sash-overlay" id="pvcResiDoorSashOverlay"></div>
                <div class="row" id="row1">
                    <div class="row-measure first">
                        <span class="cap-bottom"></span>
                        <input type="text" value="0" onchange="pvcResiDoor_updateHeight(1, this.value)">
                    </div>
                    <div class="row-panel">
                        <div class="pvc-resi-door-frame-topEdge" id="pvcResiDoorFrameTopEdge" style="cursor:pointer;" title="Click to add trickle vent"></div>
                        <!-- Trickle Vent -->
                        <div class="pvc-resi-door-trickle-vent" id="pvcResiDoorVent"></div>
                        <!-- Frame corner toggle (larger clickable area) -->
                        <div class="frame-corner-toggle" id="frameCornerToggle"></div>
                        <!-- Frame corners (top) -->
                        <div class="pvc-resi-door-frame-corner-line top-left">
                            <svg width="100%" height="100%"><line x1="0" y1="0" x2="100%" y2="100%" stroke="#a0a0a0" stroke-width="1"/></svg>
                        </div>
                        <div class="pvc-resi-door-frame-corner-line top-right">
                            <svg width="100%" height="100%"><line x1="0" y1="100%" x2="100%" y2="0" stroke="#a0a0a0" stroke-width="1"/></svg>
                        </div>
                        <div class="pvc-resi-door-frame-leftEdge"></div>
                        <div class="sash-area pvc-resi-door-sash-panel1">
                            <!-- Door Head Trickle Vent -->
                            <div class="pvc-resi-door-sash-vent" id="pvcResiDoorSashVent"></div>
                            <!-- Door head trickle hit area -->
                            <div id="pvcResiDoorSashHeadHit" style="position:absolute;left:0;top:0;width:100%;height:calc(var(--sash-inset) + var(--sash-border));z-index:600;cursor:pointer;" title="Click to add door head trickle vent"></div>
                            <!-- Sash corner toggle (larger clickable area) -->
                            <div class="sash-corner-toggle" id="sashCornerToggle"></div>
                            <!-- Sash external corners (top) -->
                            <div class="pvc-resi-door-sash-corner-line top-left">
                                <svg width="100%" height="100%"><line x1="0" y1="0" x2="100%" y2="100%" stroke="#a0a0a0" stroke-width="1"/></svg>
                            </div>
                            <div class="pvc-resi-door-sash-corner-line top-right">
                                <svg width="100%" height="100%"><line x1="0" y1="100%" x2="100%" y2="0" stroke="#a0a0a0" stroke-width="1"/></svg>
                            </div>
                            <div class="glass-area" id="glassPanel1">
                                <div class="glass-pane"></div>
                                <div class="mullion">
                                    <div class="mullion-ext top">
                                        <svg viewBox="0 0 18 9" preserveAspectRatio="none" style="width:100%; height:100%; display:block;">
                                            <line x1="0" y1="9" x2="9" y2="0" stroke="#a0a0a0" stroke-width="1"/>
                                            <line x1="18" y1="9" x2="9" y2="0" stroke="#a0a0a0" stroke-width="1"/>
                                        </svg>
                                    </div>
                                    <div class="mullion-ext bottom">
                                        <svg viewBox="0 0 18 9" preserveAspectRatio="none" style="width:100%; height:100%; display:block;">
                                            <line x1="0" y1="0" x2="9" y2="9" stroke="#a0a0a0" stroke-width="1"/>
                                            <line x1="18" y1="0" x2="9" y2="9" stroke="#a0a0a0" stroke-width="1"/>
                                        </svg>
                                    </div>
                                    <div class="rail-joint-toggle mullion-toggle" id="railJointToggleMullion"></div>
                                </div>
                                <div class="glass-pane"></div>
                                <div class="mullion">
                                    <div class="mullion-ext top">
                            ...
                            <!-- Midrail 1 bottom half weld ends -->
                            <div class="rail-joint-toggle midrail-toggle" id="railJointToggleMidrail"></div>
                            <div class="midrail-end left bottom">
                                <svg viewBox="0 0 15 9" preserveAspectRatio="none">
                                    <line x1="100%" y1="0" x2="50%" y2="100%" stroke="#a0a0a0" stroke-width="1"/>
                                </svg>
                            </div>
                            <div class="midrail-end right bottom">
                                <svg viewBox="0 0 15 9" preserveAspectRatio="none">
                                    <line x1="0" y1="0" x2="50%" y2="100%" stroke="#a0a0a0" stroke-width="1"/>
                                </svg>
                            </div>
                            <!-- Sash external corners (bottom) - shown when row1 is bottom -->
                            <div class="pvc-resi-door-sash-corner-line bottom-left bottom-row-only">
                                <svg width="100%" height="100%"><line x1="0" y1="100%" x2="100%" y2="0" stroke="#a0a0a0" stroke-width="1"/></svg>
                            </div>
                            <div class="pvc-resi-door-sash-corner-line bottom-right bottom-row-only">
                                <svg width="100%" height="100%"><line x1="0" y1="0" x2="100%" y2="100%" stroke="#a0a0a0" stroke-width="1"/></svg>
                            </div>
                        </div>
                        <div class="pvc-resi-door-frame-rightEdge"></div>
                        <!-- Frame bottom - shown when row1 is bottom -->
                        <div class="pvc-resi-door-frame-bottomEdge bottom-row-only"></div>
                        <!-- Frame corners (bottom) - shown when row1 is bottom -->
                        <div class="pvc-resi-door-frame-corner-line bottom-left bottom-row-only">
                            <svg width="100%" height="100%"><line x1="0" y1="100%" x2="100%" y2="0" stroke="#a0a0a0" stroke-width="1"/></svg>
                        </div>
                        <div class="pvc-resi-door-frame-corner-line bottom-right bottom-row-only">
                            <svg width="100%" height="100%"><line x1="0" y1="0" x2="100%" y2="100%" stroke="#a0a0a0" stroke-width="1"/></svg>
                        </div>
                    </div>
                </div>
                <div class="row" id="row2">
                    <div class="row-measure">
                        <span class="cap-bottom"></span>
                        <input type="text" value="0" onchange="pvcResiDoor_updateHeight(2, this.value)">
                    </div>
                    <div class="row-panel">
                        <div class="pvc-resi-door-frame-leftEdge"></div>
                        <div class="sash-area pvc-resi-door-sash-panel2">
                            <!-- Midrail 1 top half weld ends -->
                            <div class="midrail-end left top">
                                <svg viewBox="0 0 15 9" preserveAspectRatio="none">
                                    <line x1="100%" y1="100%" x2="50%" y2="0" stroke="#a0a0a0" stroke-width="1"/>
                                </svg>
                            </div>
                            <div class="midrail-end right top">
                                <svg viewBox="0 0 15 9" preserveAspectRatio="none">
                                    <line x1="0" y1="100%" x2="50%" y2="0" stroke="#a0a0a0" stroke-width="1"/>
                                </svg>
                            </div>
                            <div class="glass-area" id="glassPanel2">
                                <div class="glass-pane"></div>
                            </div>
                            <!-- Midrail 2 bottom half weld ends -->
                            <div class="midrail-end left bottom">
                                <svg viewBox="0 0 15 9" preserveAspectRatio="none">
                                    <line x1="100%" y1="0" x2="50%" y2="100%" stroke="#a0a0a0" stroke-width="1"/>
                                </svg>
                            ...
                            <!-- Sash external corners (bottom) - shown when row2 is bottom -->
                            <div class="pvc-resi-door-sash-corner-line bottom-left bottom-row-only">
                                <svg width="100%" height="100%"><line x1="0" y1="100%" x2="100%" y2="0" stroke="#a0a0a0" stroke-width="1"/></svg>
                            </div>
                            <div class="pvc-resi-door-sash-corner-line bottom-right bottom-row-only">
                        ...
                        <div class="pvc-resi-door-frame-rightEdge"></div>
                        <!-- Frame bottom - shown when row2 is bottom -->
                        <div class="pvc-resi-door-frame-bottomEdge bottom-row-only"></div>
                        <!-- Frame corners (bottom) - shown when row2 is bottom -->
                        <div class="pvc-resi-door-frame-corner-line bottom-left bottom-row-only">
                            <svg width="100%" height="100%"><line x1="0" y1="100%" x2="100%" y2="0" stroke="#a0a0a0" stroke-width="1"/></svg>
                        </div>
                        <div class="pvc-resi-door-frame-corner-line bottom-right bottom-row-only">
                ...
                <div class="row" id="row3">
                    <div class="row-measure">
                        <span class="cap-bottom"></span>
                        <input type="text" value="0" onchange="pvcResiDoor_updateHeight(3, this.value)">
                    </div>
                    <div class="row-panel">
                        <div class="pvc-resi-door-frame-leftEdge"></div>
                        <div class="sash-area pvc-resi-door-sash-panel3">
                            <!-- Midrail 2 top half weld ends -->
                            <div class="midrail-end left top">
                                <svg viewBox="0 0 15 9" preserveAspectRatio="none">
                                    <line x1="100%" y1="100%" x2="50%" y2="0" stroke="#a0a0a0" stroke-width="1"/>
                                </svg>
                            ...
                            <div class="glass-area" id="glassPanel3">
                                <div class="glass-pane"></div>
                            </div>
                            <!-- Sash external corners (bottom) -->
                            <div class="pvc-resi-door-sash-corner-line bottom-left">
                                <svg width="100%" height="100%"><line x1="0" y1="100%" x2="100%" y2="0" stroke="#a0a0a0" stroke-width="1"/></svg>
                            </div>
                            <div class="pvc-resi-door-sash-corner-line bottom-right">
                                <svg width="100%" height="100%"><line x1="0" y1="0" x2="100%" y2="100%" stroke="#a0a0a0" stroke-width="1"/></svg>
                            </div>
                        </div>
                        <div class="pvc-resi-door-frame-rightEdge"></div>
                        <div class="pvc-resi-door-frame-bottomEdge"></div>
                        <!-- Frame corners (bottom) -->
                        <div class="pvc-resi-door-frame-corner-line bottom-left">
                            <svg width="100%" height="100%"><line x1="0" y1="100%" x2="100%" y2="0" stroke="#a0a0a0" stroke-width="1"/></svg>
                        </div>
                        <div class="pvc-resi-door-frame-corner-line bottom-right">
                            <svg width="100%" height="100%"><line x1="0" y1="0" x2="100%" y2="100%" stroke="#a0a0a0" stroke-width="1"/></svg>
                        </div>
                    </div>
                </div>
                
                <!-- Cill row (sits below frame) -->
                <div class="cill-row" id="cillRow">
                    <div class="cill-measure"></div>
                    <div class="cill" id="doorCill"></div>
                </div>
            </div>
            
            <div class="right-measure" id="rightMeasure">
                <span class="cap-bottom"></span>
                <input type="text" value="2100" onchange="pvcResiDoor_updateTotalHeight(this.value)">
            </div>
        </div>
    </div>
</div>
    ...
    <script>
        ...
        function pvcResiDoor_recalcScaleDeps() {
            ...
            var mw  = MULLION_WIDTH_MM   / SCALE_FACTOR;
        ...
        // Mullion extension height — half the midrail thickness, capped at 45mm.
        // Defaults to 45mm if no midrail is set.
        function pvcResiDoor_updateMullionExtHeight() {
            var midrailMm = pvcResiDoor_midrail1Height > 0 ? pvcResiDoor_midrail1Height : 90;
        ...
        function pvcResiDoor_enterEmbeddedMode() {
            ...
            // Remove wrapper padding — fill the iframe fully
            ...
            // Body fills the iframe
            document.body.style.background = 'transparent';

            // Disable auto-scale resize observer — parent sets dimensions via CSS
        ...
        // Row lock state — keyed by row number (1-based). true = locked.
        // Locked rows stay fixed when total height changes.
        // Cleared only on full reset.
        var pvcResiDoor_lockedRows = {};
        // Mullion thickness per panel in mm — defaults to MULLION_WIDTH_MM
        var pvcResiDoor_mullionThicknessMm = [90, 90, 90];
        // First mullion thickness change syncs all panels. After that, each panel is independent.
        var pvcResiDoor_mullionsSynced = false;
        
        // Door open direction: 'out' (towards viewer) or 'in' (away from viewer)
        var pvcResiDoor_openDirection = 'out';
        ...
        // Midrail heights (in mm, 0 = off)
        var pvcResiDoor_midrail1Height = 0;
        var pvcResiDoor_midrail2Height = 0;
        // First midrail thickness change syncs both midrails. After that, each is independent.
        var pvcResiDoor_midrailsSynced = false;
        
        // Panel materials: 'glass' or 'panel'
        var pvcResiDoor_panelMaterials = ['glass', 'glass', 'glass'];
        
        // Pane selection and bars
        ...
        function pvcResiDoor_updateHeight(rowNum, value) {
            ...
            if (ROW_COUNT > 1) {
                // Distribute remaining height only among unlocked rows (excluding changed row)
                var lockedTotal = 0;
                var unlockedRows = [];
        ...
        function pvcResiDoor_applyMidrailHeights() {
            // Midrail half px = (midrailMm / SCALE_FACTOR) / 2
            var half1Px = pvcResiDoor_midrail1Height > 0 ? (pvcResiDoor_midrail1Height / SCALE_FACTOR) / 2 : 0;
            var half2Px = pvcResiDoor_midrail2Height > 0 ? (pvcResiDoor_midrail2Height / SCALE_FACTOR) / 2 : 0;
        ...
        function pvcResiDoor_rebuildPanelMullions(panelIndex, mullionCount) {
            ...
            var thicknessPx = thicknessMm / SCALE_FACTOR;
        ...
        window.addEventListener('message', function(event) {
            ...
            if (event.data.source === 'bubble-control' && event.data.command === 'enterEmbeddedMode') {
                pvcResiDoor_enterEmbeddedMode();
                return;
            }

            if (event.data.source === 'bubble-control' && event.data.command === 'updateDimensions') {
        ...
        var pvcResiDoor_suppressClick = false;
        ...
        function pvcResiDoor_onDrawMouseUp(e) {
            ...
            if (isH) {
                ...
                if (pvcResiDoor_midrail1Height === 0) {
                    // First midrail — snap to centre if within 15%
                    var centre    = totalHeightMm / 2;
                    var threshold = totalHeightMm * 0.15;
                    if (Math.abs(splitMm - centre) <= threshold) {
                        splitMm = centre;
                    }
                    pvcResiDoor_updateMidrail1Height(90); // default thickness
                    ...
                    // Second midrail — redistribute to equal thirds
                    pvcResiDoor_updateMidrail2Height(90); // default thickness
        ...
        function pvcResiDoor_buildMidrailDragHandles() {
            ...
            if (pvcResiDoor_midrail1Height > 0) {
                ...
                if (sash1) {
                    ...
                    handle.style.cssText = 'height:' + (half * 2 + 4) + 'px;bottom:' + (-half - 2) + 'px;';
            ...
            if (pvcResiDoor_midrail2Height > 0) {
                ...
                if (sash2) {
                    ...
                    handle2.style.cssText = 'height:' + (half2 * 2 + 4) + 'px;bottom:' + (-half2 - 2) + 'px;';
